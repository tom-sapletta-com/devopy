...
logger = logging.getLogger("devopy.dependency-manager")
...
class DependencyManager:
    ...
    def install_missing_packages(self, modules: Set[str], venv_path: Optional[str] = None) -> List[str]:
        """
        Instaluje brakujące pakiety Python
        
        Args:
            modules: Zbiór modułów do zainstalowania
            venv_path: Opcjonalna ścieżka do środowiska wirtualnego
            
        Returns:
            List[str]: Lista zainstalowanych pakietów
        """
        installed = []
        
        for module in modules:
            # Pomiń standardowe moduły
            ...
            try:
                ...
                logger.debug(f"Moduł {module} jest już zainstalowany")
            except ImportError:
                # Instaluj brakujący moduł
                logger.info(f"Instalowanie brakującego modułu: {module}")
                ...
                try:
                    if venv_path:
                        ...
                        cmd = [str(pip_path), "install", module]
                    else:
                    ...
                    logger.info(f"Zainstalowano moduł: {module}")
                except subprocess.CalledProcessError as e:
                    logger.error(f"Błąd podczas instalacji modułu {module}: {e}")
...
# Funkcja pomocnicza do instalacji brakujących pakietów
def install_missing_packages(code: str, venv_path: Optional[str] = None) -> List[str]:
    """
    Analizuje kod i instaluje brakujące pakiety
    ...
    """
    manager = DependencyManager()
    modules = manager.analyze_code(code)
    return manager.install_missing_packages(modules, venv_path)


if __name__ == "__main__":
    # Prosty interfejs wiersza poleceń
    if len(sys.argv) > 1:
        # Jeśli podano argument, traktuj go jako kod do analizy
        code = sys.argv[1]
        
        # Jeśli argument wygląda jak ścieżka do pliku, odczytaj plik
        ...
        # W przeciwnym razie wyświetl pomoc
        print("Użycie: python dependency_manager.py <kod_lub_plik>")
        print("Analizuje kod Python i dodaje brakujące importy.")
